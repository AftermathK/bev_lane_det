...
model_path = '/home/dfpazr/Documents/CogRob/avl/DSM/network_estimation/bev_lane_det/apollo_checkpoints/latest.pth'
...
def test_epoch(model, dataset, optimizer, configs, epoch):
    ...
    input_data, gt_seg_data, gt_emb_data, offset_y_data, z_data, image_gt_segment, image_gt_instance) in enumerate(
        ...
        seg = bev_pred[0].detach().cpu().numpy()
        embedding = bev_pred[1].detach().cpu().numpy()
        offset_y = torch.sigmoid(bev_pred[2]).detach().cpu().numpy()
        z_pred = bev_pred[3].detach().cpu().numpy()
